...
namespace MVC.Demo
{
	public class PoliceAISiren : ToolkitBehaviourExtension<Vehicle>
	{
		...
		public class LightRenderer
		{
			...
			public MeshRenderer Renderer
			{
				...
				set
				{
					...
					{
						ToolkitDebug.Warning("Cannot assign new the Mesh Renderer to this vehicle as it's not a part of its body or wheel models.", vehicle.gameObject);
		...
		public void Restart()
		{
			...
			{
				ToolkitDebug.Error($"The `Police AI` component has to have a `Vehicle` and/or a `Vehicle AI` component attached to the same GameObject!", gameObject);

				return;
			}
			else if (SirenClips.Length < 1)
			{
				ToolkitDebug.Warning($"We couldn't start the `Police AI` component for \"{name}\" because the `Siren Clips` list is empty!", gameObject);
